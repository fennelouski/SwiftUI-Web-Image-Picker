...
**URL-based (recommended for most apps):** depend on the repo root and a [SemVer tag](https://github.com/fennelouski/SwiftUI-Web-Image-Picker/tags) (e.g. `1.5.0`):

```swift
dependencies: [
    .package(name: "WebImagePicker", url: "https://github.com/fennelouski/SwiftUI-Web-Image-Picker.git", from: "1.5.0"),
]
```

A rule such as `from: "1.0.0"` still resolves any compatible **1.x** release (including `1.5.0`). After a new tag is published:
...
- **Pinned revision / exact version** — bump your rule (for example to `from: "1.5.0"`) and resolve again.
...
**`maximumDiscoveredImagesPerPage`** — Optional cap on how many image candidates are kept from **each** loaded page after discovery (default `nil` = unlimited). Truncation keeps the first N URLs in extractor order. For **`.staticHTML`**, that order is: `<img>` / `srcset` and `<picture>` sources in DOM order, then Open Graph and Twitter image tags, then `url(...)` values from inline `style` attributes and `<style>` blocks. In **multi-URL** mode (requires **`isMultiplePageEntryEnabled`**: primary field + `additionalPageURLs` + extra rows), the limit applies **per page** before results are merged and de-duplicated across pages.

**Cache policy** — Set **`cachePolicy`** (``WebImagePickerCachePolicy``) to tune how aggressively the picker reuses work:
- **`requestCachePolicy`** is applied to every `URLRequest` the package builds (HTML fetches, ranged GET probes, final image downloads). It composes with the integrator’s ``WebImagePickerConfiguration/urlSession``: if that session’s `URLSessionConfiguration` has a **disk** `urlCache`, responses may still be written to disk per system rules—sensitive URLs can end up in that cache. Use an **ephemeral** `URLSession` (or a custom configuration with a memory-only / disabled `URLCache`) when you need to avoid on-disk HTTP caching; the default policy uses **`useProtocolCachePolicy`**, matching behavior from before this option existed.
- The same struct optionally enables an **in-memory** cache of **raw** per-page discovery results (the image URL list from the extractor) with an LRU cap, optional TTL, and optional per-registrable-host cap. That cache lives in the process only; it is cleared when the user taps **Change URL**. Sorting, type filters, dimension filtering, and per-page caps always re-run, so changing those settings does not require a new HTML fetch. Presets: **`.ephemeral`** (default, no discovery memo) and **`.sessionBounded`** (32 entries, 10 minute TTL, 8 entries per host). Tuning: raise **`maximumDiscoveryEntries`** for long multi-URL sessions; add **`perDomainMaximumEntries`** so one site cannot crowd out others; set **`discoveryEntryTimeToLive`** to drop stale lists.
